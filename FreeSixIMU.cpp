...
void FreeSixIMU::getRawValues(float * raw_values) {
	int q[6];
  acc.readAccel(&q[0], &q[1], &q[2]);
  raw_values[0]=((float) q[0]);
  raw_values[1]=((float) q[1]);
  raw_values[2]=((float) q[2]);
  gyro.readGyroRaw(&q[3], &q[4], &q[5]);
  raw_values[3]=((float) q[3]);
  raw_values[4]=((float) q[4]);
  raw_values[5]=((float) q[5]);
